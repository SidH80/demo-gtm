import React from 'react';
import HorizontalTable from './HorizontalTable';

export default {
    title: 'RCL/Components/HorizontalTable',
    component: HorizontalTable,
    parameters: {
        docs: {
            description: {
                component:
                    'This component is designed to create a table with header Columns and data Rows',
            },
        },
    },
};

export const DefaultHorizontal = () => {
    const headers = [
        'Tax Period',
        'Reason For Payment',
        'Tax Type',
        'Amount Owed',
    ];

    const data = [
        [2018, 'Tax Return or Notice', '1040, 1040A, 1040EZ', '$959.74'],
        [2017, 'Tax Return or Notice', '1040, 1040A, 1040EZ', '$666.23'],
        [2016, 'Tax Return or Notice', '1040, 1040A, 1040EZ', '$14.74'],
        [2015, 'Tax Return or Notice', '1040, 1040A, 1040EZ', '$1020.23'],
    ];
    return <HorizontalTable headerCols={headers} dataRows={data} boldHeader />;
};
